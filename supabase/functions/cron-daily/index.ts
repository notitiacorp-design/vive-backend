...
// âââ Step 1: Process pending jobs ââââââââââââââââââââââââââââââââââââââââââââ
async function processPendingJobs(
  ...
  for (const job of jobs) {
    ...
    if (job.type === 'jarvis-engine') targetUrl = JARVIS_ENGINE_URL;
      ...
      await supabase
        .from('jobs')
        .update({ status: 'failed', attempts: newAttempts, updated_at: new Date().toISOString(), error_message: 'Unknown job type' })
        .eq('id', job.id);
      failed++;
      continue;
    }
...
// âââ Step 2: Launch jarvis-engine for active users âââââââââââââââââââââââââââ
async function launchJarvisForActiveUsers(
  ...
  // Find paid users who have recent health data
  ...
  // Get user IDs who have health data in last 24h
  ...
  const healthUserIds = [...new Set((healthUsers ?? []).map((h) => h.user_id))];
  ...
  // Exclude users already analyzed today
  ...
  const analyzedToday = new Set((alreadyAnalyzed ?? []).map((a) => a.user_id));
  const usersToAnalyze = healthUserIds.filter((id) => !analyzedToday.has(id));

  console.log(
    `[cron-daily] Paid users with health data: ${healthUserIds.length}, ` +
    `already analyzed today: ${analyzedToday.size}, ` +
    `to analyze: ${usersToAnalyze.length}`
  );
...
// âââ Step 3: Generate morning briefing notifications âââââââââââââââââââââââââ
async function generateMorningBriefings(
  ...
  // Fetch fresh analyses for these users
...
// âââ Step 4: Check box locks (J-2) âââââââââââââââââââââââââââââââââââââââââââ
async function checkBoxLocks(
  ...
  // Only lock if we are 2 days or fewer from month end
  if (daysUntilMonthEnd > 2) {
    console.log(`[cron-daily] Not yet J-2. No boxes to lock today.`);
    return { boxes_locked: 0 };
  }

  // Find pending validation boxes for current month
  const currentMonthStart = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1)).toISOString();
  const currentMonthEnd = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() + 1, 0, 23, 59, 59)).toISOString();
  ...
  const boxIds = boxesToLock.map((b) => b.id);
  ...
  const lockEvents = (updatedBoxes ?? []).map((b) => ({
    box_order_id: b.id,
...
serve(async (req: Request) => {
  ...
  // ââ Auth validation ââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  const authHeader = req.headers.get('Authorization') ?? '';
  const cronSecretHeader = req.headers.get('x-cron-secret') ?? '';
  const bearerToken = authHeader.replace(/^Bearer\s+/i, '').trim();

  const isValidServiceRole = bearerToken === SUPABASE_SERVICE_ROLE_KEY && SUPABASE_SERVICE_ROLE_KEY !== '';
  const isValidCronSecret = CRON_SECRET !== '' && cronSecretHeader === CRON_SECRET;

  if (!isValidServiceRole && !isValidCronSecret) {
  ...
  // ââ Build service-role client ââââââââââââââââââââââââââââââââââââââââââââ
  const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
    auth: { persistSession: false, autoRefreshToken: false },
  });

  // ââ Execute steps with individual error isolation ââââââââââââââââââââââââ
  const summary = {
    jobs_processed: 0,
    jobs_failed: 0,
    analyses_run: 0,
  ...
  // ââ Persist run log ââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  try {
