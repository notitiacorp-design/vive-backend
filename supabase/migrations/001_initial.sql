...
DECLARE
  ...
  v_new_level     int;
BEGIN
  -- Upsert user_levels row if it doesn't exist yet
  INSERT INTO public.user_levels (user_id, total_xp, current_level)
  VALUES (NEW.user_id, 0, 1)
  ON CONFLICT (user_id) DO NOTHING;

  -- Atomically add the XP amount (can be negative for penalties)
  UPDATE public.user_levels
  SET
    total_xp      = GREATEST(0, total_xp + NEW.amount),
    updated_at    = now()
  WHERE user_id = NEW.user_id
  RETURNING total_xp INTO v_total_xp;

  -- Recompute level
  v_new_level := fn_compute_level(v_total_xp);
...
-- ---------------------------------------------------------------------------
-- RLS Policies: jobs (restricted - service role only for most ops)
-- ---------------------------------------------------------------------------
